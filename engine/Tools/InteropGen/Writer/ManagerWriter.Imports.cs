...
#pragma warning restore CS0162
                ...
                StartBlock($"internal static class {InternalNative}");
                {
                    ...
                    foreach (Variable f in c.Variables)
                    {
                        ...
                        string managedArgsGet = $"{string.Join(", ", managedArgsGetter)}";
                        string managedArgsSet = $"{string.Join(", ", managedArgsSetter)}";

                        WriteLine($"internal static delegate* unmanaged[SuppressGCTransition]<{managedArgsGet}> Get__{f.MangledName};\n");
                        WriteLine($"internal static delegate* unmanaged[SuppressGCTransition]<{managedArgsSet}, void> Set__{f.MangledName};\n");
                    }
                }
                EndBlock();
            }
